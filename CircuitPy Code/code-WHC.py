# Jasmin's ~Official~ 3-DoF Hoxel Control Code -- WHC Demo Version
...
kZ = 1.0
scale = 4.0
def moveHoxel0(X0, Y0, Z0, cubeID0):
    if Z0 <= min_force:
        exhaustHoxel0()
    else:
        if cubeID0 == "blue":  # Normal force only
            z0(duty2bits(get_pump_Speed(kZ*Z0)))
        elif cubeID0 == "red":  # X is dominant -> Move X
            if X0 >= 0.0:
                x0_pos(duty2bits(kZ*get_pump_Speed(Z0)))
            else:
                x0_neg(duty2bits(kZ*get_pump_Speed(Z0)))
        elif cubeID0 == "green":  # Y is dominant -> Move Y
            if Y0 >= 0.0:
                y0_pos(duty2bits(kZ*get_pump_Speed(Z0)))
            else:
                y0_neg(duty2bits(kZ*get_pump_Speed(Z0)))
        elif cubeID0 == "none":
            exhaustHoxel0()
#         else:
#             if cubeID0 == "red":  # X is dominant -> Move X
#                 if X0 >= 0.0:
#                     x0_pos(duty2bits(kZ*get_pump_Speed(magF0)))
#                 else:
#                     x0_neg(duty2bits(kZ*get_pump_Speed(magF0)))
#             elif cubeID0 == "green":  # Y is dominant -> Move Y
#                 if Y0 >= 0.0:
#                     y0_pos(duty2bits(kZ*get_pump_Speed(magF0)))
#                 else:
#                     y0_neg(duty2bits(kZ*get_pump_Speed(magF0)))
#             elif cubeID0 == "none":
#                 exhaustHoxel0()


def moveHoxel1(X1, Y1, Z1, cubeID1):
    if Z1 <= min_force:
        exhaustHoxel1()
    else:
        if cubeID1 == "blue":  # Normal force only
            z1(duty2bits(kZ*get_pump_Speed(Z1)))
        else:
            if cubeID1 == "red":  # X is dominant -> Move X
                if X1 >= 0.0:
                    x1_pos(duty2bits(kZ*get_pump_Speed(Z1)))
                else:
                    x1_neg(duty2bits(kZ*get_pump_Speed(Z1)))
            elif cubeID1 == "green":  # Y is dominant -> Move Y
                if Y1 >= 0.0:
                    y1_pos(duty2bits(kZ*get_pump_Speed(Z1)))
                else:
                    y1_neg(duty2bits(kZ*get_pump_Speed(Z1)))
            elif cubeID1 == "none":
                exhaustHoxel1()
...
#         cubeID0 = data_list[10]
#         cubeID1 = data_list[11]

        # Hoxel 0:
        moveHoxel0(X0, Y0, Z0, cubeID0)
        # Hoxel 1:
        moveHoxel1(X1, Y1, Z1, cubeID1)
