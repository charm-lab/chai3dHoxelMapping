...
# Define Pump
pres_1 = AnalogIn(board.A0)
pump_control1 = board.D2
...
pwm_pump = pwmio.PWMOut(board.D13, frequency=8000, duty_cycle=0)
...
def Z0_axis_pos(pump_val):
    pwm_pump.duty_cycle = duty2bits(pump_val)
    valve1.value = True
    valve2.value = True
    valve3.value = True
    valve4.value = True
...
# Turn off Hoxel0
def hoxel0Off():
    pwm_pump.duty_cycle = duty2bits(20)
    valve1.value = False
    valve2.value = False
    valve3.value = False
    valve4.value = False
...
# Start vacuum
valve1c.value = True
pwm_pump.duty_cycle = duty2bits(100)
time.sleep(3)
