function h = meansgraph(gmeans, glower, gupper, gnames, mname, control)
...
% See if there is a control group (Dunnett's test)
iscontrol = (nargin>=6) && ~isempty(control);
if ~iscontrol
    control = 1; % default for initial selection
end
...
% Make comparison lines around the first or control group
x = [0.5 n+0.5];
x = [x NaN x];
...
for j=1:n
   ...
   h1 = plot(gmeans(j),n+1-j,'bo', 'UserData',j);
   h2 = plot(yci,      xci,  'b-', 'UserData',-j);
   if ~iscontrol
       h1.ButtonDownFcn = ftxt;
       h2.ButtonDownFcn = ftxt;
   end
end
