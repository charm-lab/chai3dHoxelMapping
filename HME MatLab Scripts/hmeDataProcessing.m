...
if (showInidividualSubjects == false)
    ...
    [indexP3] = ...
        createErrorBarPlot(indexPathLengthMeanStats,...
    ...
    [thumbP3] = ...
        createErrorBarPlot(thumbPathLengthMeanStats,...
    ...
    [boxP3] = createErrorBarPlot(boxPathLengthMeanStats,...
    ...
    [indexTest, thumbTest, boxTest] = ...
        createPathLengthsPlotTestPhase(indexP3,thumbP3,boxP3,...
   ...
   improvePlot_v2(false, true, 22, 1200, 650); hold off;
    ...
    for j = 1:numSubjects
        ...
        improvePlot_v2(false, true, 20, 700, 450); hold off;
        legend([indexTest(1), thumbTest(1), boxTest(1)],...
            "Index", "Thumb ", "Cube",...
            "Location", "northoutside", "NumColumns", 3,...
            "FontSize", 16);
...
% Looking at Total Box Breaks in the Testing Phase for each Mapping:
for j = 1:numSubjects
    totalBoxBreaksMapping1(j) = sum(numBoxBreaksMapping1{j,2});
    totalBoxBreaksMapping3(j) = sum(numBoxBreaksMapping3{j,2});
    totalBoxBreaksMapping5(j) = sum(numBoxBreaksMapping5{j,2});
end

 % plot(subjectNum,totalBoxBreaksMapping1,'ro');
 % hold on; 
 % plot(subjectNum,totalBoxBreaksMapping3,'go');
 % plot(subjectNum,totalBoxBreaksMapping5,'bo');
 close all;
 figure;
    ...
    for j=1:numSubjects
        ...
        if (saveFigures == true)
            ...
            print(gcf,...
                ...
                'Subject',num2str(subjectNum(j)),...
                '_avgTimeBoxBroken'),'-dpdf','-r0'); %close;
        end
    end
...
[p_CompletionTime, stats_CompletionTime] = ...
    runLeveneTest(completionTimeMapping1, completionTimeMapping3,...
    completionTimeMapping5, "Completion Time");
%% Path Lengths: *********************************************************
close all;
% Index
[p_indexPathLength, stats_indexPathLength] = ...
    runLeveneTest(indexPathLengthMapping1, indexPathLengthMapping3,...
    indexPathLengthMapping5, "Index Path Length");

% Thumb
[p_thumbPathLength, stats_thumbPathLength] = ...
    runLeveneTest(thumbPathLengthMapping1, thumbPathLengthMapping3,...
    thumbPathLengthMapping5, "Thumb Path Length");

% Box
[p_boxPathLength,stats_boxPathLength] = ...
    runLeveneTest(boxPathLengthMapping1, boxPathLengthMapping3,...
    boxPathLengthMapping5, "Cube Path Length");
...
[p_indexNormal, stats_indexNormal] = ...
    runLeveneTest(meanIndexNormalForceMapping1, meanIndexNormalForceMapping3,...
    meanIndexNormalForceMapping5, "Index Normal Force");
% Shear
[p_indexShear, stats_indexShear] = ...
    runLeveneTest(meanIndexShearForceMapping1, meanIndexShearForceMapping3,...
    meanIndexShearForceMapping5, "Index Shear Force");
...
[p_thumbNormal, stats_thumbNormal] = ...
    runLeveneTest(meanThumbNormalForceMapping1, meanThumbNormalForceMapping3,...
    meanThumbNormalForceMapping5, "Thumb Normal Force");
% Shear
...
[p_CompletionTime, ~ , stats_CompletionTime] = ...
    runHMEANOVA2(completionTimeMapping1, completionTimeMapping3,...
    completionTimeMapping5, "Completion Time");
...
% Index
[p_indexPathLength, ~ , stats_indexPathLength] = ...
    runHMEANOVA2(indexPathLengthMapping1, indexPathLengthMapping3,...
    indexPathLengthMapping5, "Index Path Length");

% Thumb
[p_thumbPathLength, ~ , stats_thumbPathLength] = ...
    runHMEANOVA2(thumbPathLengthMapping1, thumbPathLengthMapping3,...
    thumbPathLengthMapping5, "Thumb Path Length");

% Box
[p_boxPathLength, ~ , stats_boxPathLength] = ...
    runHMEANOVA2(boxPathLengthMapping1, boxPathLengthMapping3,...
    boxPathLengthMapping5, "Cube Path Length");
...
% Normal
[p_indexNormal, ~ , stats_indexNormal] = ...
    runHMEANOVA2(meanIndexNormalForceMapping1, meanIndexNormalForceMapping3,...
    meanIndexNormalForceMapping5, "Index Normal Force");
% Shear
[p_indexShear, ~ , stats_indexShear] = ...
    runHMEANOVA2(meanIndexShearForceMapping1, meanIndexShearForceMapping3,...
    meanIndexShearForceMapping5, "Index Shear Force");
...
% Normal
[p_thumbNormal, ~ , stats_thumbNormal] = ...
    runHMEANOVA2(meanThumbNormalForceMapping1, meanThumbNormalForceMapping3,...
    meanThumbNormalForceMapping5, "Thumb Normal Force");
% Shear
[p_thumbShear, ~ , stats_thumbShear] = ...
    runHMEANOVA2(meanThumbShearForceMapping1, meanThumbShearForceMapping3,...
    meanThumbShearForceMapping5, "Thumb Shear Force");
