...
void haptics_thread::UpdateVRGraphics()
{
    ...
    chai3d::cVector3d box3Pos = p_CommonData->ODEBody3->getLocalPos();
    ...
    if(p_CommonData->currentEnvironmentState == dynamicBodies)
    {
        ...
        if (p_CommonData->currentDynamicObjectState == HoxelMappingExperiment)
        {
            ...
            if (p_CommonData->enablePlanarConstraint == true)
            {
                ...
                chai3d::cVector3d thisPos = p_CommonData->ODEBody1->getLocalPos(); //box1Pos
            ...
            //Box to wire line
            /** Find min distance between box and line and show -- CURRENTLY NOT WORKING: **/
            /*
            //boxToWireLine->m_pointA = box1Pos;
            //double normU = cDistance(wireEnd1, wireEnd2);
            //boxToWireLine->m_pointB = box1Pos + cDiv(normU, cNormalize(cCross(wireEnd1 - box1Pos, wireEnd2 - wireEnd1))) ;
            chai3d::cVector3d A = box1Pos - wireEnd1;
            chai3d::cVector3d B = wireEnd2 - wireEnd1;

            double u_length = cDot(A,B)/ B.length();

            chai3d::cVector3d u = wireEnd1*u_length;

            boxToWireLine->m_pointA = box1Pos;
            boxToWireLine->m_pointB = u;

            //TEMP:
            boxToWireLine->m_pointA = box1Pos;
            boxToWireLine->m_pointB = wire->getLocalPos();
            */
        }
...
//    UpdateScaledBoxes();
    ...
    if(p_CommonData->currentDynamicObjectState == CrumblyCubeExperiment)
    {
        ...
        //Find distance between box1 and hoop2
    ...
    if (p_CommonData->whcDemo == true)
    {
        ...
        chai3d::cVector3d indexToCube1Dist = box1Pos - finger->getLocalPos();
        chai3d::cVector3d indexToCube2Dist = box2Pos - finger->getLocalPos();
        chai3d::cVector3d indexToCube3Dist = box3Pos - finger->getLocalPos();

        chai3d::cVector3d thumbToCube1Dist = box1Pos - thumb->getLocalPos();
        chai3d::cVector3d thumbToCube2Dist = box2Pos - thumb->getLocalPos();
        chai3d::cVector3d thumbToCube3Dist = box3Pos - thumb->getLocalPos();

        if(indexToCube1Dist.length() <= radius*1.25*boxSize1)
        {
            p_CommonData->cubeID0 = "red";
        }
        else if(indexToCube2Dist.length() <= radius*1.25*boxSize1)
        {
            p_CommonData->cubeID0 = "green";
        }
        else if(indexToCube3Dist.length() <= radius*1.25*boxSize1)
        {
            p_CommonData->cubeID0 = "blue";
        }
        else
        {
            p_CommonData->cubeID0 = "none";
        }

        if(thumbToCube1Dist.length() <= radius*1.25*boxSize1)
        {
            p_CommonData->cubeID1 = "red";
        }
        else if(thumbToCube2Dist.length() <= radius*1.25*boxSize1)
        {
            p_CommonData->cubeID1 = "green";
        }
        else if(thumbToCube3Dist.length() <= radius*1.25*boxSize1)
        {
            p_CommonData->cubeID1 = "blue";
        }
        else
        {
...
void haptics_thread::InitFingerAndTool()
{
    ...
    */
    ...
    scaledFingerColor.setBrownSaddle();
    ...
    scaledThumbColor.setBrownSaddle();
...
void haptics_thread::InitDynamicBodies()
{
    ...
    backWall = new chai3d::cMesh(); //Added for HME and FME v2
    sideWall1 = new chai3d::cMesh(); //Added for HME and FME v2
    sideWall2 = new chai3d::cMesh(); //Added for HME and FME v2
    ...
    boxToTargetIdealPath = new chai3d::cShapeLine(chai3d::cVector3d(0,0,0),
                                                  chai3d::cVector3d(0,0,0));
...
void haptics_thread::RenderDynamicBodies()
{
    ...
    //Ground dimensions:
    double groundDimX = 1.75*groundLength;
    double groundDimY = 2.5*groundLength;
    ...
    */
    ...
    case CrumblyCubeExperiment:  // Jasmin CrumblyCube Experiment
        ...
        stiffness1 =  p_CommonData->stiffness1; stiffness2 =  p_CommonData->stiffness2; stiffness3 = p_CommonData->stiffness3;
        break;
    case CubeGuidanceExperiment:  // Jasmin CubeGuidance Experiment
        boxSize1 = 0.04; boxSize2 = 0.04; boxSize3 = 0.04;
        friction1 = EXPERIMENTFRICTION; friction2 = EXPERIMENTFRICTION; friction3 = EXPERIMENTFRICTION;
        mass1 = p_CommonData->mass1; mass2 = p_CommonData->mass2; mass3 = p_CommonData->mass3;
        stiffness1 =  p_CommonData->stiffness1; stiffness2 =  p_CommonData->stiffness2; stiffness3 = p_CommonData->stiffness3;
...
*/
    ...
    wireLengthInM = 0.500; //the wire is 500mm long
    wireEnd1 = wire->getLocalPos() + chai3d::cVector3d(0.0, 0.5*wireLengthInM, 0.0);
    wireEnd2 = wire->getLocalPos() + chai3d::cVector3d(0.0, -0.5*wireLengthInM, 0.0);

    //Line for min distance box to wire
    //Define lines for showforces
    /*chai3d::cColorf boxtoWireLineColor;
    boxtoWireLineColor.setPinkHot();
    boxToWireLine = new chai3d::cShapeLine(chai3d::cVector3d(0,0,0),
                                         chai3d::cVector3d(0,0,0));
    boxToWireLine->setLineWidth(5);
    boxToWireLine->m_colorPointA=boxtoWireLineColor;
    boxToWireLine->m_colorPointB=boxtoWireLineColor;
    p_CommonData->p_world->addChild(boxToWireLine);
*/
    p_CommonData->target1Complete = false;
    p_CommonData->hoop1Complete = false;
...
void haptics_thread::SetDynEnvironAdjust() //susana change other properties here
{
    ...
    p_CommonData->ODEBody1->setLocalPos(0.25, -0.15, -0.3); //red
    p_CommonData->ODEBody2->setLocalPos(0.20, 0.10, -0.2); //green
    p_CommonData->ODEBody3->setLocalPos(0.15, 0.35, -0.1); //blue
    ...
    //    matHoop2.setWhite();
    //    hoop2->setMaterial(matHoop2);
    //    // p_CommonData->p_world->addChild(hoop2);  // Remove for WHC Demo
...
void haptics_thread::SetManualAdjust() //susana change other properties here
{
    // create the visual boxes on the dynamicbox meshes
    cCreateBox(p_CommonData->p_dynamicBox1, boxSize1, boxSize1, boxSize1); // make mesh a box
    //    cCreateBox(p_CommonData->p_dynamicBox2, boxSize1*0.9, boxSize1*1.05, boxSize1*0.9); // make mesh a box
    //    cCreateBox(p_CommonData->p_dynamicBox2, boxSize2, boxSize2, boxSize2); // make mesh a box
    //    cCreateBox(p_CommonData->p_dynamicBox3, boxSize3, boxSize3, boxSize3); // make mesh a box
    ...
    */
    ...
    // set position of box
    p_CommonData->ODEBody1->setLocalPos(0.15, -0.2, -0.3);
    //    p_CommonData->ODEBody2->setLocalPos(0.2, -0.1, -0.2);
    //    p_CommonData->ODEBody3->setLocalPos(0.25, 0.0, -0.1);
    ...
    double sc = 0.6;
    chai3d::cCreateEllipsoid(target3, targetRadius*sc, targetRadius*sc, targetRadius*sc);
    target3Pos = chai3d::cVector3d(0.05, 0.18, 0.0);
    target3->setLocalPos(0.0, 0.0, 0.0); //target3Pos.x(), target3Pos.y(), target3Pos.z());
    matTarget3.setRed();
    target3->setMaterial(matTarget3);
    target3->setUseCulling(true);
    target3->setUseTransparency(true);
