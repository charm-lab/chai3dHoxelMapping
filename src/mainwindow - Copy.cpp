    ...
    ui(new Ui::MainWindow)
{
    ...
    //windowGLDisplay->setGeometry(QRect(0, 0, 1275, 1000));
...
void MainWindow::UpdateGUIInfo()
{
    ...
    ui->progressBar->setValue(abs(100*((double)(localOutputVoltages0[0])))/.96);
    ui->progressBar_2->setValue(abs(100*((double)(localOutputVoltages0[1])))/.96);
    ui->progressBar_3->setValue(abs(100*((double)(localOutputVoltages0[2])))/.96);
    ...
    ui->progressBar_4->setValue(abs(100*((double)(localOutputVoltages1[0])))/.96);
    ui->progressBar_5->setValue(abs(100*((double)(localOutputVoltages1[1])))/.96);
    ui->progressBar_6->setValue(abs(100*((double)(localOutputVoltages1[2])))/.96);
    ...
    ui->lcdSubjective_Pair_Num->display(p_CommonData->Subjective_Pair_Num);
    ...
    //Vibration
    ui->VibrationOutput_lcd->display(p_CommonData->texture);

    #ifdef OCULUS
    ui->lcdCurrentMass->display(p_CommonData->InertiaBlockMass);
    ui->lcdNumber_SF->display(p_CommonData->Inertial_Scaling_Factor);
    ui->lcdNumber_ref->display(p_CommonData->ReferenceMass);
    ui->lcdNumber_comp->display(p_CommonData->ComparisonMass);
    #endif

    //ui->VibrationOutput_lcd->display(p_CommonData->VibrationOutput);
    ...
    if(ui->ImpulseVib->isChecked())
        p_CommonData->ImpulseVib = true;
    else
        p_CommonData->ImpulseVib = false;

    if(ui->SDVib->isChecked())
        p_CommonData->SDVib = true;
    else
        p_CommonData->SDVib = false;

    if(ui->JustVib->isChecked())
        p_CommonData->JustVib = true;
    else
        p_CommonData->JustVib = false;

    if(ui->Decaying->isChecked())
        p_CommonData->Decaying = true;
    else
        p_CommonData->Decaying = false;
...
bool MainWindow::readExpStuffIn(){
    p_CommonData->TrialType = p_CommonData->InertiaProtocolFile.GetValue((QString("trial ") + QString::number(p_CommonData->trialNo)).toStdString().c_str(), "type", NULL /*default*/);
    if (p_CommonData->TrialType=="trial"){
        p_CommonData->currentExperimentState = InertialscalingTrial;
        p_CommonData->ReferenceMass=atoi(p_CommonData->InertiaProtocolFile.GetValue((QString("trial ") + QString::number(p_CommonData->trialNo)).toStdString().c_str(), "Reference", NULL /*default*/));
        p_CommonData->ComparisonMass=atoi(p_CommonData->InertiaProtocolFile.GetValue((QString("trial ") + QString::number(p_CommonData->trialNo)).toStdString().c_str(), "Comparisons", NULL /*default*/));
        p_CommonData->Inertial_Scaling_Factor=.1*atoi(p_CommonData->InertiaProtocolFile.GetValue((QString("trial ") + QString::number(p_CommonData->trialNo)).toStdString().c_str(), "Scaling", NULL /*default*/));
        p_CommonData->ReferenceFirst=atoi(p_CommonData->InertiaProtocolFile.GetValue((QString("trial ") + QString::number(p_CommonData->trialNo)).toStdString().c_str(), "ReferenceFirst", NULL /*default*/));
        p_CommonData->fileName = QString::number(p_CommonData->Inertial_Scaling_Factor*10);
        return true;
    }else if(p_CommonData->TrialType=="break"){
        p_CommonData->currentExperimentState = trialBreak;
        if (p_CommonData->blockNo==0){
            p_CommonData->blockSize=p_CommonData->trialNo; //sets block size
        }
        p_CommonData->blockNo+=1;
        return false;
    }else if(p_CommonData->TrialType=="end"){
        p_CommonData->currentExperimentState = endExperiment;
        qDebug()<<"done!!!!!!!!!!!!!!!!!!!!!!!!!!!!";
        return false;
    }else if(p_CommonData->TrialType=="training"){
        p_CommonData->currentExperimentState = training;
        p_CommonData->ReferenceMass=atoi(p_CommonData->InertiaProtocolFile.GetValue((QString("trial ") + QString::number(p_CommonData->trialNo)).toStdString().c_str(), "Reference", NULL /*default*/));
        p_CommonData->ComparisonMass=atoi(p_CommonData->InertiaProtocolFile.GetValue((QString("trial ") + QString::number(p_CommonData->trialNo)).toStdString().c_str(), "Comparisons", NULL /*default*/));
        p_CommonData->Inertial_Scaling_Factor=.1*atoi(p_CommonData->InertiaProtocolFile.GetValue((QString("trial ") + QString::number(p_CommonData->trialNo)).toStdString().c_str(), "Scaling", NULL /*default*/));
        p_CommonData->ReferenceFirst=atoi(p_CommonData->InertiaProtocolFile.GetValue((QString("trial ") + QString::number(p_CommonData->trialNo)).toStdString().c_str(), "ReferenceFirst", NULL /*default*/));
...
void MainWindow::keyPressEvent(QKeyEvent *a_event)
{
    ...
    //Inertia Experiment Buttons
    if(a_event->key() == Qt::Key_P)
    {
        if (p_CommonData->currentExperimentState == trialBreak){
            ...
            WhatMassAreWeDisplaying();
            if (readExpStuffIn()){
            }

            if (p_CommonData->ReferenceFirst == 1){
                p_CommonData->InertiaBlockMass = p_CommonData->ReferenceMass/1000;
            }
            else {
                p_CommonData->InertiaBlockMass = p_CommonData->ComparisonMass*p_CommonData->Inertial_Scaling_Factor/1000;
            }
//            qDebug()<< "Trial NO" <<p_CommonData->trialNo<<" pairNo" <<p_CommonData->pairNo;
//            qDebug()<< "Trial type "<<p_CommonData->TrialType;
//            qDebug()<< "ReferenceMass" <<p_CommonData->ReferenceMass;
//            qDebug()<< "ComparisonMass" <<p_CommonData->ComparisonMass;
//            qDebug()<< "InteriaL Scaling Factor" <<p_CommonData->Inertial_Scaling_Factor;
//            qDebug()<< "Reference First" <<p_CommonData->ReferenceFirst;
            WhatMassAreWeDisplaying();
            p_CommonData->environmentChange = true;
            UpdateGUIInfo();
        }else if (p_CommonData->currentExperimentState == endExperiment){
    ...
    if(a_event->key() == Qt::Key_H) //progressing to next pair
    {
        ...
        if (p_CommonData->currentDynamicObjectState == dynamicSubjectiveExp){
             if(CheckFingers()&& (p_CommonData->fingerTouching == false && p_CommonData->thumbTouching == false)){
              if(p_CommonData->Subjective_Pair_Num==1){
                  p_CommonData->Subjective_Pair_Num=0;
                  p_CommonData->environmentChange = true;
                  UpdateGUIInfo();
              }else
              {
                  p_CommonData->Subjective_Pair_Num=1;
                  p_CommonData->environmentChange = true;
                  UpdateGUIInfo();
              }
             }
        ...
        if (p_CommonData->currentDynamicObjectState == dynamicInertiaExp){
            if (p_CommonData->currentExperimentState == InertialscalingTrial || p_CommonData->currentExperimentState == training){
                if(CheckFingers()&& (p_CommonData->fingerTouching == false && p_CommonData->thumbTouching == false)){
                    if (p_CommonData->trialNo == 0){ //Starting first trial
                        p_CommonData->dataRecordMutex.lock();
                        p_CommonData->dataRecorderVector.clear();
                        p_CommonData->dataRecordMutex.unlock();
                        p_CommonData->pairNo=1;
...
//                        qDebug()<< "Reference First" <<p_CommonData->ReferenceFirst;
                        ...
                        if (p_CommonData->pairNo==1){
                            p_CommonData->pairNo=2; //progressing
                            if (p_CommonData->ReferenceFirst == 1){ //if we just showed the reference
                                p_CommonData->InertiaBlockMass = p_CommonData->ComparisonMass*p_CommonData->Inertial_Scaling_Factor/1000;
                            }
                            else { //if we just showed comparison
                                p_CommonData->InertiaBlockMass = p_CommonData->ReferenceMass/1000;
                            }
                            WhatMassAreWeDisplaying();
                            p_CommonData->environmentChange = true;
                            ...
                            p_CommonData->pairNo=1; //progressing
                            if (p_CommonData->ReferenceFirst == 1){ //if we just showed the reference
                                p_CommonData->InertiaBlockMass = p_CommonData->ReferenceMass/1000;
                            }
                            else { //if we just showed comparison
    ...
    if(a_event->key() == Qt::Key_Y) //answered first block was heavier
    {
          UpdateGUIInfo();
    }
    if(a_event->key() == Qt::Key_J) //answered first block was heavier
    {
        if (p_CommonData->currentDynamicObjectState == dynamicInertiaExp){
            if (p_CommonData->currentExperimentState == InertialscalingTrial || p_CommonData->currentExperimentState == training){
                if(CheckFingers()&& (p_CommonData->fingerTouching == false && p_CommonData->thumbTouching == false)){
                    if (p_CommonData->pairNo == 2){
                        ...
                        p_CommonData->dataRecorder.subjResponse = p_CommonData->subjectAnswer;
                        ...
                        p_CommonData->dataRecorderVector.clear();
                        p_CommonData->dataRecordMutex.unlock();
                        if(p_CommonData->currentExperimentState != training){
                        WriteDataToFile();
                        }
                        qDebug()<<"First Mass Chosen";
                        p_CommonData->trialNo+=1;
                        p_CommonData->pairNo=1;
                        if (readExpStuffIn()){
...
//                            qDebug()<< "Reference First" <<p_CommonData->ReferenceFirst;
    ...
    if(a_event->key() == Qt::Key_K) //answered second block was heavier
    {
            ...
            if (p_CommonData->currentExperimentState == InertialscalingTrial || p_CommonData->currentExperimentState == training){
                if(CheckFingers() && (p_CommonData->fingerTouching == false && p_CommonData->thumbTouching == false)){
                    if (p_CommonData->pairNo == 2){
                        ...
                        qDebug()<<"Second Mass Chosen";

                        p_CommonData->trialNo+=1;
                        p_CommonData->pairNo=1;

                        if (readExpStuffIn()){
                            if (p_CommonData->ReferenceFirst == 1){ //if we have to show the reference
                                p_CommonData->InertiaBlockMass = p_CommonData->ReferenceMass/1000;
                            }
                            else { //if we have to show the reference
                                p_CommonData->InertiaBlockMass = p_CommonData->ComparisonMass*p_CommonData->Inertial_Scaling_Factor/1000;
                            }
//                            qDebug()<< "Trial NO" <<p_CommonData->trialNo<<" pairNo" <<p_CommonData->pairNo;
//                            qDebug()<< "ReferenceMass" <<p_CommonData->ReferenceMass;
...
void MainWindow::WriteDataToFile()
{
    ...
    char trialBuffer[33];
    double file_number = p_CommonData->trialNo - p_CommonData->blockSize*p_CommonData->blockNo;
    ...
    for (int i=0; i < localDataRecorderVector.size(); i++)
    {
        ...
        file <<std::setprecision(9)<< localDataRecorderVector[i].time << "," << " "
             ...
             << localDataRecorderVector[i].InertiaScalingFactor << "," << " "
             ...
             << localDataRecorderVector[i].blockNo << "," << " "
             << localDataRecorderVector[i].ReferenceMass << "," << " "
             << localDataRecorderVector[i].ComparisonMass << "," << " "
             << localDataRecorderVector[i].boxrotation(0,0) << "," << " "
             << localDataRecorderVector[i].boxrotation(0,1) << "," << " "
             << localDataRecorderVector[i].boxrotation(0,2) << "," << " "
             << localDataRecorderVector[i].boxrotation(1,0) << "," << " "
             << localDataRecorderVector[i].boxrotation(1,1) << "," << " "
             << localDataRecorderVector[i].boxrotation(1,2) << "," << " "
             << localDataRecorderVector[i].boxrotation(2,0) << "," << " "
             << localDataRecorderVector[i].boxrotation(2,1) << "," << " "
             << localDataRecorderVector[i].boxrotation(2,2) << "," << " "
...
void MainWindow::on_dynamicEnvironment_clicked()
{
    ...
    p_CommonData->InertiaBlockMass=.2;
...
{
    //Open dialog box to get protocol file and save into variable
    QString temp = "C:/Users/Sam/Dropbox (Stanford CHARM Lab)/Sam Schorr Research Folder/New Tactile Feedback Device/Protocol Creation/Experiments/SizeWeight/Subjects/Subject_001/Protocol.ini";//QFileDialog::getOpenFileName();
...
void MainWindow::WhatMassAreWeDisplaying(){
    if (p_CommonData->pairNo == 1 && p_CommonData->ReferenceFirst == 1){
        p_CommonData->Displaying=1; //Reference
        //qDebug() << "Reference";
    } else if (p_CommonData->pairNo == 2 && p_CommonData->ReferenceFirst == 1){
        p_CommonData->Displaying=0; //Comparison
        //qDebug() << "Comparison";
    } else if (p_CommonData->pairNo == 1 && p_CommonData->ReferenceFirst == 0){
        p_CommonData->Displaying=0; //Comparison
        //qDebug() << "Comparison";
    } else if (p_CommonData->pairNo == 2 && p_CommonData->ReferenceFirst == 0){
        p_CommonData->Displaying=1; //Reference
        //qDebug() << "Reference";
    };
...
void MainWindow::on_InertialMassSlider_valueChanged(int value)
{

    p_CommonData->Inertial_Scaling_Mass=value;
    if ( p_CommonData->currentDynamicObjectState == standard){
        if ( p_CommonData->currentEnvironmentState == dynamicBodies){

            // p_CommonData->ODEBody1->setMass(((11.0-p_CommonData->Inertial_Scaling_Factor)/10)*p_CommonData->Inertial_Scaling_Mass/1000);
        }
    }

}

void MainWindow::on_InertialFactorSlider_valueChanged(int value)
{
    //p_CommonData->Inertial_Scaling_Factor=value;
    if ( p_CommonData->currentDynamicObjectState == standard){
        if ( p_CommonData->currentEnvironmentState == dynamicBodies){
            //p_CommonData->ODEAdjustBody->setMass(((11.0-p_CommonData->Inertial_Scaling_Factor)/10)*p_CommonData->Inertial_Scaling_Mass/1000);
        }
    }
}


void MainWindow::on_StartInertiaTrial_clicked()
{
    ui->JakeRenderCheckBox->setChecked(true);
    p_CommonData->environmentChange = true;
    p_CommonData->currentEnvironmentState = dynamicBodies;
    p_CommonData->currentExperimentState = InertialscalingTrial;
    p_CommonData->currentDynamicObjectState = dynamicInertiaExp;
    p_CommonData->currentControlState = VRControlMode;
    p_CommonData->dataRecordMutex.lock();
    p_CommonData->dataRecorderVector.clear();
    p_CommonData->dataRecordMutex.unlock();
    p_CommonData->InertiaBlockMass=.2;
    p_CommonData->Inertial_Scaling_Factor=1;
    p_CommonData->Inertial_Scaling_Mass=200;
    p_CommonData->pairNo = 1;
    p_CommonData->trialNo = 0;
    p_CommonData->ReferenceMass=200;
    p_CommonData->expCD=1;
    p_CommonData->blockNo=0;
    p_CommonData->blockSize=0;
    ui->VRControl->setChecked(true);

}

void MainWindow::on_SetIDir_clicked(){
    //Open dialog box to get protocol file and save into variable
    //QString temp = "C:/Users/Sam/Desktop/Subjects/InertiaExp/Subjects/Subject_003/Protocol.ini";
    //QString temp = QFileDialog::getOpenFileName();
    //QString temp = "C:/Users/Sam/Dropbox (Stanford CHARM Lab)/Sam Schorr Research Folder/New Tactile Feedback Device/Protocol Creation/Experiments/SizeWeight/Subjects/Subject_008/Protocol.ini";


    p_CommonData->dir = QFileDialog::getExistingDirectory(0, "Select Directory for file",
                                                          "C:/Users/Sam/Desktop/Subjects/InertiaExp/Subjects",
                                                          QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);


    QString temp = QFileDialog::getOpenFileName(this,"C:/Users/Sam/Desktop/Subjects/InertiaExp/Subjects");
    //QString temp = "C:/Users/Sam/Dropbox (Stanford CHARM Lab)/Sam Schorr Research Folder/New Tactile Feedback Device/Protocol Creation/Experiments/SizeWeight/Subjects/Subject_008/Protocol.ini";

    p_CommonData->sizeWeightProtocolLocation = temp;
    int error = p_CommonData->InertiaProtocolFile.LoadFile(temp.toStdString().c_str());
    qDebug() << "error" << error << p_CommonData->sizeWeightProtocolLocation;
...
void MainWindow::on_ContinueInertiaTrial_clicked()
{
    ui->JakeRenderCheckBox->setChecked(true);
    ...
    p_CommonData->dataRecordMutex.unlock();
    p_CommonData->pairNo = 1;
    p_CommonData->trialNo = ui->TrialNum->toPlainText().toDouble();
    p_CommonData->blockNo = ui->BlockNo->toPlainText().toDouble();
    p_CommonData->blockSize = ui->BlockSize->toPlainText().toDouble();
...
void MainWindow::on_Subjective_Exp_clicked()
{
    ui->JakeRenderCheckBox->setChecked(true);

    p_CommonData->currentEnvironmentState = dynamicBodies;
    p_CommonData->currentExperimentState = SubjectiveTrial;
    p_CommonData->currentDynamicObjectState = dynamicSubjectiveExp;
    p_CommonData->currentControlState = VRControlMode;
     p_CommonData->Subjective_Pair_Num =1;
...
void MainWindow::on_stiffnessSlider_valueChanged(int value)
{
        p_CommonData->adjustedStiffness=value;
        p_CommonData->environmentChange = true;

}


void MainWindow::on_VibrationButton_clicked()
{
